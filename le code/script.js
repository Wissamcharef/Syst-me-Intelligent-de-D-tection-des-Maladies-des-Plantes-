...
let currentLang = localStorage.getItem("plantLang") || "fr";
...
function updateLanguage() {
  ...
  // Update button text
  document.getElementById("lang-btn-desktop").textContent =
    currentLang === "fr" ? "EN" : "FR";
  document.getElementById("lang-btn-mobile").textContent =
    currentLang === "fr" ? "English" : "Français";

  // Nav
  document.getElementById("nav-home").textContent = t.home;
  document.getElementById("nav-home-mobile").textContent = t.home;
  if (document.getElementById("nav-diagnostic"))
    document.getElementById("nav-diagnostic").textContent = t.diagnostic;
  if (document.getElementById("nav-diagnostic-mobile"))
    document.getElementById("nav-diagnostic-mobile").textContent = t.diagnostic;
  if (document.getElementById("nav-history"))
    document.getElementById("nav-history").textContent = t.history;
  if (document.getElementById("nav-history-mobile"))
    document.getElementById("nav-history-mobile").textContent = t.history;
  document.getElementById("nav-auth-text").textContent = currentUser
    ? t.logout
    : t.login;
  document.getElementById("nav-auth-mobile").textContent = currentUser
    ? t.logout
    : t.login + " / " + t.register;

  // Home
  document.getElementById("home-title-1").textContent = t.intelligentDetection;
  document.getElementById("home-title-2").textContent = t.plantDiseases;
  document.getElementById("home-description").textContent = t.homeDesc;
  document.getElementById("start-analysis-btn").textContent = t.startAnalysis;
  document.getElementById("stat-accuracy").textContent = t.accuracy;
  document.getElementById("stat-diseases").textContent = t.diseases;
  document.getElementById("stat-time").textContent = t.responseTime;
  ...
  document.getElementById("step-4-text").textContent = t.step4;

  // Auth
  document.getElementById("tab-login").textContent = t.login;
  document.getElementById("tab-register").textContent = t.register;
  document.getElementById("login-submit").textContent = t.loginBtn;
  document.getElementById("accept-terms").innerHTML = t.acceptTerms.replace(
    "conditions d'utilisation",
    '<a href="#" class="text-[#486F38] hover:underline">conditions d\'utilisation</a>'
  );

  // History
  document.getElementById(
    "clear-history-btn"
  ).innerHTML = `<i data-lucide="trash-2" class="w-4 h-4"></i> ${t.clearHistory}`;
  document.getElementById("new-analysis-empty").textContent = t.newAnalysisBtn;
  document.getElementById("label-analyses").textContent = t.analyses;
  document.getElementById("label-member").textContent = t.memberSince;
}
...
const diseaseDatabase = [
  ...
  },
  {
    ...
    confidence: 89.7,
    description:
      "Taches circulaires grises entourées d'un halo brun sur les feuilles. Progression rapide par temps humide.",
  ...
  },
  {
    ...
    confidence: 96.3,
    description:
      "Virus provoquant une mosaïque jaune-verte sur les feuilles, déformation et rabougrissement.",
    treatment:
      "Aucun traitement curatif. Arracher et détruire les plants infectés.",
    prevention:
      "Lavage des mains et outils. Résistance variétale. Lutte anti-pucerons.",
  ...
  },
  {
    ...
    confidence: 97.8,
    description:
      "Taches nécrotiques entourées de halos jaunes. Contamination par les outils et éclaboussures.",
    treatment:
      "Cuivre organique. Streptomycine (si autorisé). Destruction des plants.",
...
function simulateAnalysisSteps() {
  const t = translations[currentLang];
  const steps = [
    { id: "step-1", status: t.step1 + "...", delay: 0 },
    { id: "step-2", status: t.step2 + "...", delay: 1000 },
    {
      id: "step-3",
      status:
        "ResNet50 " +
        (currentLang === "fr" ? "classification..." : "classification..."),
      delay: 2000,
    },
    { id: "step-4", status: t.step4 + "...", delay: 3000 },
  ];
...
function displayResults(result, file) {
  ...
  document.getElementById("processing-time").textContent =
    "Traité en " + (1.2 + Math.random()).toFixed(1) + "s";
  ...
  severityEl.className =
    ...
    (result.severity === "Sévère"
      ? "bg-red-100 text-red-700"
      : result.severity === "Modérée"
...
function generateSimilarDiseases(currentResult) {
  ...
  const others = diseaseDatabase
    .filter((d) => d.name !== currentResult.name)
    .slice(0, 3);
  others.forEach((disease) => {
    const div = document.createElement("div");
    div.className =
      "border rounded-xl p-4 hover:shadow-md transition cursor-pointer";
    div.innerHTML = `
        ...
        item.name
          ...
          <div class="p-4">
              <div class="flex items-center justify-between mb-2">
                  ...
                  <span class="px-2 py-0.5 rounded text-xs font-medium ${getSeverityColor(
                    item.severity
                  )}">
                      ${item.severity}
                  </span>
              ...
              <button onclick="viewHistoryDetail(${
                item.id
              })" class="w-full py-2 border border-emerald-600 text-emerald-600 rounded-lg text-sm font-medium hover:bg-emerald-50 transition">
                  Voir détails
              </button>
          </div>
      </div>
...
function showUserProfile() {
  ...
  document.getElementById("user-since").textContent = new Date(
...
function logout() {
  ...
  document
    ...
    .classList.add("text-emerald-600", "border-emerald-600");
  document
    .getElementById("tab-login")
    .classList.remove("text-gray-400", "border-transparent");
  document
    .getElementById("tab-register")
    .classList.remove("text-emerald-600", "border-emerald-600");
  document
    .getElementById("tab-register")
    .classList.add("text-gray-400", "border-transparent");
...
function checkAuthState() {
  ...
  // Always show home by default unless already logged in and coming back
  if (!currentUser) {
    showSection("home");
  }
}
